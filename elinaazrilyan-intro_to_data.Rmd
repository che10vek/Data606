...
# library(DATA606)
# setwd("/Users/elinaazrilyan/Documents/Fall 2018/Data 606/Week 2/Lab1/")
source("more/cdc.R")
```
...
```{r rowscount, eval=TRUE}
nrow(cdc)
```

Determining number of variables:
```{r columnscount, eval=TRUE}
ncol(cdc)
```
...
2.  Create a numerical summary for `height` and `age`, and compute the 
    ...
    #23.29% report being in excellent health
    ```
...
<span style="color:red">Answer:</span> It looks like males seems to be more likely to smoke that females but the difference is not very drastic.
...
#Commenting out this code - to make this lab more readable
#cdc[,6]
```
...
<span style="color:red">Answer:</span> Based on this plot it looks like higher BMI seems to correlate with poor health.

```{r}
boxplot(bmi ~ cdc$exerany)
```

<span style="color:red">Answer:</span> The variable I chose is Exercise, I want to see if exercising correlates with lower BMI. The plot seems to suggest a slight correlation. but is not as significant as I expected.
...
<span style="color:red">Answer:</span> The 2nd graph is much more descriptive - the 1st histogram group data into very large range - so it is not easy to read the results.
...
<span style="color:red">Answer:</span> It looks like there is a positive relationship between the variables.
...
<span style="color:red">Answer:</span> 'wdiff' is a continuos numerical variable. If 'wdiff' is 0 that means that the person's actual and desired weight are equal - they are at their desired weight. Is it is positive that means that are hoping to gain weight and it is negative it means that they are hoping to lose weight. 
...
<span style="color:red">Answer:</span> It looks like the general trend is that people are hoping to lose weight.
...
<span style="color:red">Answer:</span> It looks like men feel like they are closer to their desired weight then women. 
...
highend <- mw + sdw
highend
weightinrange <- subset(cdc, weight > lowend & weight < highend)
nrow(weightinrange)
nrow(weightinrange)/20000
```
<div id="license">
