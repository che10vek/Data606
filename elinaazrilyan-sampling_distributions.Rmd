...
The center is closer to true mean and the spread is smaller.
...
```{r sampprice, eval=TRUE}
samppr1 <- sample(price, 50)
...
The distribution is normally distributed and the mean apears to be around 180,000, so my guess is that mean home price is around that number.
...
The results are very similar - the distribution is normally distributed and the mean looks very clearly centered close to 180,000, so my guess still stands.
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    Distibution 3 has a smaller spread, which is preferable for making better estimated.
